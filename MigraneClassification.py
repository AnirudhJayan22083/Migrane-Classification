...
svd = TruncatedSVD(n_components=10)  # Reduce to 10 dimensions (can be adjusted)
features_svd = svd.fit_transform(features_normalized)

# Split into training and testing sets (70% train, 30% test)
print('Splitting data into training and testing sets...')
X_train, X_test, y_train, y_test = train_test_split(features_svd, labels, test_size=0.3, random_state=42)

print(len(X_train), len(X_train[1]))
c=0
for i in X_train:
    if len(i) != 10:  # Check if the number of features after SVD is correct
        print(i)
    for element in i:
        if np.isnan(element):
            print("NaN found!")
